...
{{--                                    <p class="fs-medium mbpx-10">--}}
{{--                                        Use Promo Code <strong class="fc-red">OSHA21</strong> to Save--}}
{{--                                        <strong>$21</strong>--}}
{{--                                    </p>--}}
{{--                                </div>--}}
{{--                            </div>--}}
...
{{--                                            <input type="submit" class="btn --btn-primary" style="padding: 11px 20px;"--}}
{{--                                                   value="Apply">--}}
{{--                                        @else--}}
{{--                                            <input autocomplete="off" placeholder="{{ $cart->coupon->code }}"--}}
{{--                                                   readonly="readonly" class="form-field" name="coupon_code"--}}
{{--                                                   type="text">--}}
{{--                                            <a href="{{ route('coupon.remove') }}">--}}
{{--                                                <button type="button" class="removeCoupon btn --btn-danger"--}}
...
{{--                                    </div>--}}
{{--                                    @if ($coupon_error)--}}
{{--                                        <p class="coupon-code-error error-msg">Kindly try with valid coupon code.</p>--}}
{{--                                    @endif--}}
{{--                                </div>--}}
{{--                            </div>--}}
{{--                            {!! Form::close() !!}--}}
                        ...
                        <div class="cartTotalCalc">
                            ...
                            <div class="row">
                                ...
                                <span class="fs-medium">
                                    ...
                                    <strong
                                        class="float-right fs-medium fc-primary">${{ number_format($total,2) }}</strong>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-12">
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            var selectize = $("#free-course-multiselect").selectize({
                ...
                onChange: function (val) {
                    ...
                    window.free_courses = selectedItems.length > 0 ? selectedItems : []
                    // var formData = {
        ...
        $(document).ready(function () {
            ...
            if (outreachProfileCookie) {
                ...
                // Hide free course select on Manager
                var courseQty = "{{ $courseQty }}";
                if(outreachProfileCookieObj.userType == 2) {

                    $(".free-course-content").html(`
                        <span class="fs-medium">
                            You have ${courseQty} Free Course(s),
                            you can assign them from our LMS
                        </span>
                        <br/>
                    `);
                ...
                console.log(outreachProfileCookieObj, 821);
                ...
                // hide user details, show payment details
                $('.user-details').hide();
                ...
                //make steps active
                $("#step_2, #step_3").addClass("active");

                //disable click on steps
                $("#step_1").attr("id", "step_1_dis")
                $("#step_2").attr("id", "step_2_dis")
                $("#step_3").attr("id", "step_3_dis")
            ...
            async function renderStripeElements() {
                ...
                /*
                    Get Address and billing details for guest user,
                    loggedIn user will have object defined
                */
                if(!userDetailsObj) {
                ...
                $(".pay-now-btn").on("click", function (event) {
                    ...
                    payWithCard(stripe, card, order.clientSecret, elements, userDetailsObj, order.id);
                })
            ...
            var payWithCard = function (stripe, card, clientSecret, elements, userDetailsObj, orderId) {
                loading(true);

                elements.on('change', function (event) {
                    console.log(event, 971);
                })
                ...
                stripe
                    .confirmPayment({
                        elements,
                        confirmParams: {
                            return_url: "{{ url('stripe/capture-redirect-payments') }}/" + orderId,
                            shipping: {
                                address: {
                                    city: userDetailsObj.city,
                                    country: "US",
            ...
            var orderComplete = function (paymentIntentId, paymentResult) {
                ...
                console.log({...paymentResult, paymentMethod: "Stripe"}, 733)
                ...
                fetch("{{ url(env('LMS_API_URL') . '/shop/orders/') }}" + order.id + '/updatePayment', {
                    method: 'PATCH',
                    headers: {
                        'content-type': 'application/json',
                        "Accept": "application/json, text-plain, */*",
            ...
            function updateOrder() {
                ...
                if (outreachProfileCookie) {
                    ...
                    console.log(outreachProfileCookieObj, 932);

                    data.push(
                        {name: "_token", value: $("#order_details_form  input[name='_token']").val()},
                        {name: "uoid", value: $("#order_details_form  input[name='uoid']").val()},
                        {name: "user", value: outreachProfileCookieObj['_id']},
                    ...
                    if (iti.isValidNumber() == true || $('#email').val().trim() != '') {
                        ...
                        data = $('#order_details_form').serializeArray();
                ...
                formDataJSON.cart = '{!! json_encode($cart) !!}'
                formDataJSON.free_courses = window.free_courses

                // console.log(data);
                var formData = $.param(data);
                // console.log(formData);
                console.log(formDataJSON, 1122);
                return fetch("{{ url(env('LMS_API_URL') . '/shop/orders') }}", {
                    method: "PATCH",
                    headers: {
                        'content-type': 'application/json',
                        "Accept": "application/json, text-plain, */*",
                    ...
                    // })
                    // .then(function (result) {
                    //     console.log(result, 1307)
                    //     order = result;
                    //
                    //     // Hide loader after fetch
                    //     // $(".loader").hide()
                    //
                    //     if (!stripe_rendered && $('.payment-details').is(':visible')) {
                    //         renderStripeElements()
                    //     }
                    // });
            }
    ...
    <script>
        ...
        function showAlert(message, type = 'success') {
            ...
            alertDiv.empty().append('<i class="xicon icon-notification"></i>&nbsp;<strong>' + message + '</strong>');
